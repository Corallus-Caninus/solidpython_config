...
use std::write;
//TODO: obligatory "refactor this script" todo
...
fn main() {
    ...
    let mut file = File::create(format!("{}/configuration.toml", project_name))
    ...
    loop {
        ...
        io::stdin()
            ...
            .expect("Failed to read line");
        object_name = object_name.chars().filter(|c| c.clone() != '\n').collect();
        //object_name.pop();//because reasons
        //object_name.pop();
        if object_name == "done" || object_name.is_empty() || object_name == "\n" {
            break;
        } else {
    ...
    for object_name in object_names.clone() {
        print!("got object_name: {}", object_name);
